...
/* LOGIN AUTOMÁTICO (Para procesos hijos) */
int auto_login(int s) {
    char cmd[128], res[128];
    
    read(s, res, LINELEN); // Banner bienvenida
...
/* NEGOCIACIÓN MODO PASIVO (Retorna socket de datos conectado) */
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    int code = 0;
    if (sscanf(res, "%d", &code) == 1 && code == 230) break;
  ...
  while (1) {
    ...
    if (fgets(prompt, sizeof(prompt), stdin) != NULL) {
      ...
      if (strcmp(ucmd, "dir") == 0) {
            /* DIR se queda síncrono para ver resultados al instante */
            sdata = pasivo(s_main);
            if (sdata < 0) { printf("Error en PASV\n"); continue; }
            ...
            if (pid == 0) { 
                ...
                int code = 0; sscanf(res, "%d", &code);
                if (code >= 500) { printf("Error: %s\n", res); exit(1); }
            ...
            if (access(arg, R_OK) == -1) { perror("Archivo local"); continue; }
